...
export const updateViewportTiles = mutation(
    async (
        ...
        for (const upd of tileUpdates) {
            ...
            if (!blocks.has(key)) {
                const rec = await db
                    ...
                    .unique();
                if (!rec) {
                    throw new Error(`Missing block (${bx},${by}) in tileset ${tileSetId}`);
                }
                blocks.set(key, {
                    id: rec._id,
        ...
        return { updatedTiles: tileUpdates.length, updatedBlocks: [...blocks.values()].filter(b => b.modified).length };
    }
);

export const clearTileBlocksBatch = internalMutation({
    args: { tileSetId: v.id("tileSets"), cursor: v.optional(v.string()), userId: v.string() },
    handler: async ({ db, scheduler }, { tileSetId, cursor, userId }) => {
        ...
        if (!batch.isDone) {
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearTileBlocksBatch, {
                tileSetId,
...
export const clearTileSetBatch = internalMutation({
    ...
    handler: async ({ db, scheduler }, { tileSetId, userId }) => {
        const batch = await db
            .query("tileSets")
            .withIndex("byOwner", (q) => q.eq("owner", userId)).collect()
        ...
        if (wait) {
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearTileSetBatch, {
                tileSetId,
                userId,
            });
        }
...
export const clearUserDataBatch = internalMutation({
    ...
    handler: async ({ db, scheduler }, { cursor, userId }) => {
        ...
        for (const tileSet of batch.page) {
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearTileBlocksBatch, { tileSetId: tileSet._id, userId });
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearClustersBatch, { tileSetId: tileSet._id, userId });
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearTileSetBatch, { tileSetId: tileSet._id, userId });
        }

        if (!batch.isDone) {
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearUserDataBatch, {
                cursor: batch.continueCursor,
                userId,
            });
        }
...
export const clearClustersBatch = internalMutation({
    ...
    handler: async ({ db, scheduler }, { tileSetId, cursor, userId }) => {
        ...
        if (!batch.isDone) {
            await scheduler.runAfter(0, internal.functions.saveTileSet.clearClustersBatch, {
                tileSetId,
                cursor: batch.continueCursor,
                userId,
            });
...
export const clearUserData = mutation({
    handler: async ({ scheduler, auth, db }) => {
        ...
        await scheduler.runAfter(0, internal.functions.saveTileSet.clearUserDataBatch, { userId: id._id });
    },
});